...
export default function SideBar() {
  return (
    <div className="font_exo no-scrollbar Sidebar_wraper z-10 fixed top-0 left-0 px-5 py-4 h-screen  max-w-[200px] w-full overflow-y-scroll no-scrollbar dark:bg-[#191919] dark:!shadow bg-white">
      <Link href="/">
        <a className="logo_wraper inline-block">
          <Image
            className="!w-full !h-full"
            src="/ww-star-272x90.png"
            width="272"
            height="90"
            alt="logo"
          />
      ...
      <ul className="pt-10">
        {NavLinks?.map((navItem, idx) => {
          return (
            <li key={idx} className="cursor-pointer pb-6 hover:text-red-600 ">
                    ...
                    <div className="flex items-center justify-between">
                      <Link href={navItem.href}>
                        <a className="flex items-center   text-[13px] font-medium uppercase">
                          <span className="pr-1">{navItem.navIcon}</span>{" "}
                          {navItem.label}
                        </a>
                      </Link>
                                  ...
                                  <Link href={submenu?.href}>
                                    <a
                                      className="dark:text-gray-100
                                                                                    dark:hover:text-red-600 mt-3 flex items-center hover:text-red-600  text-[13px] text-gray-800 font-medium uppercase"
                                    >
                                      <span className="pr-1">
