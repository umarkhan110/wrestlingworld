    ...
    return (
        <div className='pt-2 mx-2'>
            <form onSubmit={handleSubmitSearch} className="w-full md:w-[630px] m-auto shadow-md h-[50px] rounded-md border-gray-600 dark:bg-[#191919] bg-white  flex items-center flex-1">
                ...
                <input minLength={1} type="search" onChange={(e) => setSearchInput(e.target.value)} name="search" aria-label='search' className={`font_exo placeholder:font-medium  placeholder:text-sm ${theme === 'light' ? 'text-[#444]' : 'text-[#efefef]'}   bg-transparent text-base flex-1 pr-2 outline-none pl-3 w-[90%]`} placeholder={placeHolder} />
